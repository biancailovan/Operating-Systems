...
#define ISBITSET(number, POS) ((number)&(1 << POS)) //POS, nu P0S

int generateSets(int largest_num)
{
	int i, j; //virgula intre i si j
	int maxLoop;
	...
	for (i = 0; i < maxLoop; i++) {
		printf("{");
		for (j = 0; j < largest_num; j++){ //doar un singur egal
			if (ISBITSET(i, j) != 0){
				printf("%d ", j);
			}
		}
...
int main(int argc, char** argv)
{
	...
	sscanf(argv[1], "%d", &largest_num); //&largest_num, expected type int*
	res = generateSets(largest_num); //in loc de argv[1]=>largest_num
	printf("\nExista %d submultimi\n", res); //fara &
	return 0; //0 nu O
}
